...
app.get(`/`, (req, res, next) => {
  ...
  p.image().size(`tiny`).src(`/images/example.jpg`).shadow(true);
  
  const blank = p.blank().size(`small`).addWrapperClass(`fixed`);
  
  blank.append(new ezhtml.Paragraph().text(`The picture you saw in the example image component was of my niece Molly when she was about two years old I would guess.  <img src='/images/example.jpg' class='float-right t-image-150px t-image-shadow-sm m-2'> She's always been a cutie so I figured she'd work well in the demo.`));
  blank.append(new ezhtml.Paragraph().text(`I'm including her again in this blank component that has had two paragraphs and her image appended to show how simple it is to get text and image content to flow together using simple classes like float-left and float-right.`));
  blank.append(new ezhtml.Paragraph().text(`You can also see that it's possible to just write HTML inline in the text for simple things like inline images, links, special font features, and other traditional inter-paragraph content.`));
